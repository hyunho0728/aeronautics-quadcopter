...
-- 💡 [수정] 전역 변수 대신 파일 내부에서만 접근 가능한 로컬 테이블 선언 (전역 변수 사용 금지)
local navState = {
    targetX = 0,
    targetZ = 0,
    autoFlight = false
...
local function controlLoop()
    ...
    while true do
        ...
        helpers.displayLine(monitor, 16,  string.format("Pos X:   %6.2f m",    pos.x))
        helpers.displayLine(monitor, 17,  string.format("Pos Z:   %6.2f m",    pos.z))
...
local function inputLoop()
    print("Waiting for remote altitude/position command... (CH: " .. CHANNEL .. ")")
    while true do
        ...
        if type(packet) == "table" then
            ...
            elseif packet.type == "ADD_ALT" then
                ...
                if diff then
                    TARGET_ALT = TARGET_ALT + diff
                    altPID.sp  = TARGET_ALT
                    altPID.integral   = 0
                    altPID.prev_error = 0
                    print(string.format("Received -> Adjusted Alt: %+g m -> New Target: %g m", diff, TARGET_ALT))
                end

            -- 💡 [신규 추가] 특정 좌표 이동 명령 수신 (로컬 navState 테이블 변수에 안전하게 할당)
            elseif packet.type == "MOVE_POS" then
                local tx = tonumber(packet.targetX)
                local tz = tonumber(packet.targetZ)
                
                if tx and tz then
                    navState.targetX = tx
                    navState.targetZ = tz
                    navState.autoFlight = true
                    print(string.format("Received -> Move to Position: X=%g, Z=%g (Dist: %.1f m)", tx, tz, distance or 0))
                else
                    print("Warning: Invalid position coordinates received.")
                end
...
-- 💡 외부에서 안전하게 목적지 테이블 상태를 가져가거나 수정할 수 있도록 개설한 인터페이스
local function getNavState()
    return navState
end

local function setAutoFlight(bool)
    navState.autoFlight = bool
end

local function getTargetAlt()
    return TARGET_ALT
end

local function setTargetAlt(val)
    TARGET_ALT = val
end
...
local M = {
    ...
    setAutoFlight = setAutoFlight,
    getTargetAlt = getTargetAlt,
